<script>
    ...
    function openUpdateProgress(lit) {
        newCurrentPage = lit.currentPage; // Set local state from the book entry
        progressComment = ""; // Reset comment field
    }
    ...
    try {
        ...
        // Fetch existing journalLogs and current page
        const entryDocSnap = await getDoc(entryDocRef);
        let journalLogs = [];
        let previousPage = pageStart; // Default previous page
        ...
        // Add new log entry with Date as Title
        const logEntry = {
            dateTitle: new Date().toLocaleDateString(), // Display date as title
        ...
        await updateDoc(entryDocRef, {
            ...
            updatedAt: new Date(),
            pagesRead: pagesRead, // Store total pages read
            journalLogs: journalLogs // Update logs
        });
                ...
                {:else}
                    <ul class="space-y-3">
                        ...
                        <li class="p-4 bg-white border border-neutral-300 rounded-md shadow">
                            <div class="cursor-pointer" on:click={() => lit.showLogs = !lit.showLogs}>
                                    ...
                                    <ul class="space-y-2">
                                        {#each lit.journalLogs as log}
                                            <li class="p-2 bg-white border rounded-md shadow-sm">
                                                <strong>{log.dateTitle}</strong> <!-- Date as Title -->
                                        ...
                                        <div class="flex justify-end mt-4">
                                            <Button
                                                on:click={async () => {
                                                    ...
                                                    lit.isUpdating = false;  // Close popover after saving
                                                }}
