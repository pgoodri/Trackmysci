<script>
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (type === "Ratings") {
                // Ensure all ratings (1-5) exist in the dataset
                const allRatings = ["1", "2", "3", "4", "5"];  // Ensure keys are strings
                let normalizedData = {};

                allRatings.forEach(rating => {
                    normalizedData[rating] = data[rating] || 0; // Fill in missing values
                });

                chartData.labels = Object.keys(normalizedData);
                chartData.data = Object.values(normalizedData);
            } else {
                // Normalize labels for Tags & Authors
                let normalizedData = {};
                Object.keys(data).forEach(label => {
                    const normalizedLabel = label.toLowerCase().trim();
                    normalizedData[normalizedLabel] = (normalizedData[normalizedLabel] || 0) + data[label];
                });
